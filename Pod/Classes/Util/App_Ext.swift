...
extension UIColor {
    /// RGBÍ∞? Í∞ÅÍ∞Å `red`, `green`, `blue`?ù¥Í≥? opacity?äî `a`?ù∏ Color object Î∞òÌôò
...
// MARK: - UIColor
extension UIColor {
    /// RGBÍ∞? `hex`, opacity?äî 1?óê ?ï¥?ãπ?ïò?äî Color object Î∞òÌôò
    ...
    // OpacityÍ∞íÏù¥ `opacity`?ù∏ Color object Î∞òÌôò
...
extension UITextField {
    ...
    func addDoneButtonOnKeyboard() {
        ...
        let done: UIBarButtonItem = UIBarButtonItem(title: "?ôÑÎ£?", style: .done, target: self, action: #selector(self.doneButtonAction))
...
extension String {
    ...
    // URL ?ù∏ÏΩîÎî© : ?åå?ùºÎØ∏ÌÑ∞ Î∂?Î∂ÑÎßå ?ù∏ÏΩîÎî©
    func urlEnc() -> String {
    ...
    func substring(from: Int, to: Int) -> String {
        ...
        let endIndex = index(self.startIndex, offsetBy: to + 1) // '+1'?ù¥ ?ûà?äî ?ù¥?ú†: endIndex?äî Î¨∏Ïûê?ó¥?ùò ÎßàÏ??Îß? Í∑? ?ã§?ùå?ùÑ Í∞?Î¶¨ÌÇ§Í∏? ?ïåÎ¨?
...
extension UIScrollView {
    func setCommonAttribute() {
        if self is UITableView {
            ...
            // Í∞?Î°? ?ä§?Å¨Î°? ?ù∏?îîÏº??ù¥?Ñ∞ ?†úÍ±?
            self.showsHorizontalScrollIndicator = false
        }
    }
}

// MARK: - Ï¢åÏ∏°?†ï?†¨ Ïª¨Î†â?ÖòÎ∑?
